...
func Setup(e *echo.Echo, h *Handlers, hub *ws.Hub, jwtSecret string, buildNumber string, commitSHA string) {
	...
	approved.GET("/exchange/position/:companyId", h.Exchange.GetPosition, middleware.RequireScope("read:market"))
	...
	if h.Chat != nil {
		...
		admin.POST("/chat/wiki/notion-sync-all", h.Chat.AdminSyncNotionAll)
		admin.GET("/chat/wiki/:slug", h.Chat.AdminGetWikiDoc)
		admin.PUT("/chat/wiki/:slug", h.Chat.AdminUpdateWikiDoc)
		admin.POST("/chat/wiki/:slug/notion-sync", h.Chat.AdminSyncNotionOne)
